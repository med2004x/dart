...
- sorting makes hidden order visible
- comparisons decide where each item belongs
- you need a predictable rule for ties
...
func helper() string {
    return "put the data in a predictable order"
}
...
|---|---|
| sort.Strings | applies an order rule |
| names | changes place in the slice |
| fmt.Println | shows the sorted result |
