...
- resources are nouns with ownership
- collections and single items need different URL shapes
- the model should read cleanly from the path
...
func helper() string {
    return "shape URLs around resources and IDs"
}
...
func main() {
    fmt.Println("collections use plural nouns, items use IDs")
}
```

### Expected output
```text
collections use plural nouns, items use IDs
```
...
|---|---|
| collection | lists many resources |
| single item | targets one resource |
| URL | shows ownership and shape |
