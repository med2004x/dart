...
- retry tokens or request IDs often carry the safety rule
- the server should not double-apply the same action
...
func helper() string {
    return "make safe retries possible with a request key"
}
...
func main() {
    fmt.Println("same request ID should not create two orders")
}
```

### Expected output
```text
same request ID should not create two orders
```
...
|---|---|
| request ID | marks one logical operation |
| retry | may happen more than once |
| server rule | prevents double apply |
