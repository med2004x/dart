...
- timeouts keep waits bounded
- retries change load and must be controlled
- only safe operations should be repeated automatically
...
func helper() string {
    return "bound the wait and retry only safe work"
}
...
func main() {
    fmt.Println("set a deadline, retry only safe operations")
}
```

### Expected output
```text
set a deadline, retry only safe operations
```
...
| retry | repeats safe work |
| safe operation | can happen twice |
